...
/**
 * ---------------------------------------------------------------------------
 * TEMPLATES — output-layer feature, added on top of the existing pipeline.
 *
 * A template is ONLY an ordering + a one-line framing sentence over the five
 * sections the pipeline already produces. It never changes extraction, never
 * changes what gets fetched/skeletonized/cached, and never adds per-language
 * logic. This keeps it safe to add/remove templates without touching the
...
// A section renders as empty ONLY when its body is exactly the pipeline's
// own "nothing found" placeholder for that section — never a guess, never a
// length/content heuristic. Entry points is deliberately excluded: its "no
// entry files under cap" state still carries useful skeletonized/failed
// detail, so it's never worth hiding.
...
/**
 * Render a previously-built Tier 1 result (must include `.preamble` and
 * `.sections`, both returned by buildTier1 below) under a named template.
 * Pure and synchronous — no network calls, no re-fetching, no re-extraction.
 * Safe to call as many times as you like on the same build result.
 */
...
export async function buildTier1(repoInput, token, onProgress = () => {}, cache = {}) {
  ...
  // Default assembly — SAME behavior/output shape as before templates existed.
  // Untemplated output stays the default; nothing about this path changes.
  ...
  return {
    markdown,
    // Additive fields, used only by renderWithTemplate — the raw ingredients
    // for re-assembling this same build under a different section order.
    preamble,
    sections,
    tokenEstimate,
    trimmedNote,
